...
{ /usr/bin/time -f "\t%e" ./mysql -D tpch -e 'select * from orders17 force index(o_custkey) where o_custkey between 1 and 100000;' > /dev/null; } &
{ /usr/bin/time -f "\t%e" ./mysql -D tpch -e 'select * from orders18 force index(o_custkey) where o_custkey between 1 and 100000;' > /dev/null; } &

./mysql -u root -D tpch -e 'show global status;'
